...
test_that("plm can handle different inputs", {
    ...
    expect_error(plm('Q~W',V316_river))
    expect_error(plm(V~W,V316_river))
    expect_error(plm(Q~W+X,V316_river))
    expect_error(plm(Q~W,V316_river,c_param=1.5)) # c_param higher than lowest stage measurements
    expect_error(plm(Q~W,V316_river,c_param=1L)) # c_param not double
    expect_error(plm(Q~W,V316_river,h_max=1.3)) #h_max lower than highest stage measurement
    skip_on_cran()
    V316_river_new_names <- V316_river
    names(V316_river_new_names) <- c('t1','t2')
    set.seed(1)
    plm.fit_new_names <- plm(t2~t1,V316_river_new_names,parallel=F)
...
test_that("the plm object with unknown c is in tact", {
    ...
    expect_equal(plm.fit$data,V316_river[order(V316_river$W),c('Q','W')])
})
...
test_that("the plm object with known c with a maximum stage value is in tact", {
    ...
    expect_equal(max(plm.fit_known_c$rating_curve$h),2)
