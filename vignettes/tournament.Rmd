...
summary(t_obj)
```
Notice here that in round 1, <tt>gplm</tt> is favored over <tt>gplm0</tt> in the first game and <tt>plm0</tt> over <tt>plm</tt> in the second. In the second round, <tt>gplm</tt> is deemed the tournament winner, i.e. the model that provides the best fit of the data at hand.
...
The main difference between four models lies in the modelling of the power law exponent ($f(h)$) and the variance on the data level ($\sigma^2_{\varepsilon}$). Thus it is insightful to look at the posterior of the power law exponent for the different models
```{r, fig.width=8, fig.height=6}
plot(t_obj,type='f')
```
...
There are ways to customize the tournament further. For example, if the parameter of zero discharge, $c$, is known you  might want to fix that parameter to the known value in the model. Assume 22.35 m is the known value of $c$. Then you can directly run a tournament with the $c$ parameter fixed in all models
```{r,cache=T}
t_obj_known_c <- tournament(formula=Q~W,data=kallstorp,c_param=22.35)
summary(t_obj_known_c)
```
One useful feature of the plot function is the panel option. This plots four different plots of the tournament winning model, which in this case is gplm:
```{r, fig.width=8, fig.height=6}
