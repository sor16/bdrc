...
```{r, cache=T}
set.seed(1)
t_obj <- tournament(Q~W,krokfors,parallel=T,num_cores=2) # parallel=T by default and by default, the number of cores is detected on the machine
```
...
```{r, fig.width=8, fig.height=6}
plot(t_obj,type='panel',transformed=T)
```
