...
We will dive deeper into visualizing the "bgplm" object in the next sections.
...
```{r, fig.width=8, fig.height=6}
plot(bgplm.fit,type='histogram',param='c')
```
...
```{r, fig.width=8, fig.height=6}
plot(bgplm.fit,type='histogram',param='c',transformed=T)
```
...
plot(bgplm.fit,type='histogram',param=c('a','c'))
```
There is a shorthand to visualize all hyperparameters at once
```{r, fig.width=10, fig.height=6}
plot(bgplm.fit,type='histogram',param='hyperparameters')
```
...
```{r, fig.width=10, fig.height=6}
plot(bgplm.fit,type='histogram',param='hyperparameters',transformed=T)
```

Finally, we can visualize certain components of the drc models that may vary with stage, the power-law exponent $f$ and the standard deviation on the data level $\sigma_{\varepsilon}$. bgplm0 and bgplm generalize the power law exponent, model it as a function of stage $f(h)=b+\beta(h)$, while in bplm0 and bplm model the exponent as a fixed constant $f(h)=b$. We can plot the exponent with

```{r, fig.width=8, fig.height=6}
plot(bgplm.fit,type='f')
```
bplm and bgplm model the standard deviation, $\sigma_{\varepsilon}$ on the data level as a function of stage, namely as a linear combination of B-splines functions, while bplm0 and bgplm0 model the variance as a fixed constant. We can plot the standard deviation by writing
```{r, fig.width=8, fig.height=6}
...
plot(bgplm.fit.known_c,type='collage',transformed=T)
```
...
rating_curve_h_grid <- predict(bgplm.fit.known_c,newdata=h_grid)
