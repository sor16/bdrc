...
get_report_grob <- function(...,type=1){
  ...
  if(length(args_class)!=1){
    stop(error_msg1)
  }else if(!(args_class%in%legal_types)){
    stop(error_msg1)
  }
...
get_report_pages_fun <- function(...,directory=NULL,report_title=NULL,type=1){
  ...
  if(is.null(report_title)){
    ...
    if(class(report_title)!='character'){
      stop('Report title has to be a single string')
  ...
  file_path <- paste0(directory,report_title)
...
save_report <- function(grob_list,type=type,file_path,paper='a4',width=8,height=11){
  ...
  if(type==1){
    grid.arrange(grob_list$main_page_plots[[1]],grob_list$main_page_table[[1]],nrow=2,as.table=TRUE,heights=c(5,3),top=textGrob(grob_list$obj_class,gp=gpar(fontfamily="Times",fontsize=22,facetype='bold')))
    grid.arrange(grob_list$p_mat,nrow=1,as.table=TRUE,heights=c(1),top=textGrob(paste0('Rating curve predictions for ',grob_list$obj_class),gp=gpar(fontsize=22,facetype='bold',fontfamily="Times")))
  }else{
    lapply(names(grob_list$main_page_plots),function(x){
      grid.arrange(grob_list$main_page_plots[[x]],grob_list$main_page_table[[x]],nrow=2,as.table=TRUE,heights=c(5,3),top=textGrob(x,gp=gpar(fontfamily="Times",fontsize=22,facetype='bold')))
    })
    grid.arrange(arrangeGrob(grob_list$mcmc_table,grob_list$tour_table,nrow=1),arrangeGrob(grob_list$dev_boxplot,ncol=2),nrow=2,as.table=TRUE,heights=c(1,1),top=textGrob('Model comparison and MCMC diagnostics',gp=gpar(fontsize=22,facetype='bold',fontfamily="Times")))
    grid.arrange(grob_list$conv_diag_plots,as.table=TRUE,top=textGrob('',gp=gpar(fontsize=22,facetype='bold',fontfamily="Times")))
    lapply(names(grob_list$main_page_plots), function(x) {
      grid.arrange(arrangeGrob(grobs=grob_list$mcmc_hist_list[[x]],nrow=4,ncol=3),top=textGrob(paste0('Estimated parameters of ',x),gp=gpar(fontsize=20,facetype='bold',fontfamily="Times")))
    })
  }
  dev.off()
}
