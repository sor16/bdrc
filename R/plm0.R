...
#'
#' plm0 is used to fit a discharge rating curve for paired measurements of stage and discharge using a power-law model with constant log-error variance as described in Hrafnkelsson et al. (2022). It also supports discharge measurement-error data. See "Details" for a more in depth description of the model.
#'
...
#' plm0_me.fit <- plm0(formula = Q | Q_sigma ~ W, data = provo, num_cores = 2)
#' summary(plm0_me.fit)
#'
#' # With categorical measurement error data
#' plm0_me_cat.fit <- plm0(formula = Q | Q_quality ~ W, data = mokelumne, num_cores = 2)
#' summary(plm0_me_cat.fit)
#' }
#' @export
plm0 <- function(formula, data, c_param = NULL, h_max = NULL, parallel = TRUE, num_cores = NULL, forcepoint = rep(FALSE, nrow(data)), verbose = TRUE){
    # argument checking
    ...
    # Bayesian inference
    MCMC_output_list <- plm0.inference(y = log(Q), Q_me = Q_me, h = h, c_param = c_param, h_max = h_max, parallel = parallel, forcepoint = forcepoint, num_cores = num_cores, verbose = verbose)
    ...
    if(is.null(c_param)){
        ...
        result_obj$posterior$sigma_eps <- sqrt(exp(MCMC_output_list$theta[2, ]))
    } else {
        result_obj$posterior$c <- NULL
        result_obj$posterior$sigma_eps <- sqrt(exp(MCMC_output_list$theta[1, ]))
    }
    ...
    result_obj$summary$parameters <- get_MCMC_summary(rbind(MCMC_output_list$x[1:2, ], MCMC_output_list$theta))
...
plm0.inference <- function(y, Q_me, h, c_param = NULL, h_max = NULL, parallel = TRUE, forcepoint = rep(FALSE, length(h)), num_cores = NULL, num_chains = 4, nr_iter = 20000, burnin = 2000, thin = 5, verbose){
    if(verbose) cat("Progress:\nInitializing Metropolis MCMC algorithm...\n")
    RC <- get_model_components('plm0', y, Q_me, h, c_param, h_max, forcepoint, h_min = NULL)
...
plm0.density_evaluation_unknown_c <- function(theta, RC) {
    if(is.null(RC$Q_me)){
        return(plm0_density_evaluation_unknown_c_cpp(theta = theta,
                                                     h = RC$h,
                                                     y = RC$y,
                                                     epsilon = RC$epsilon,
                                                     Sig_ab = RC$Sig_ab,
        ...
        return(plm0_me_density_evaluation_unknown_c_cpp(theta = theta,
                                                        h = RC$h,
                                                        y = RC$y,
                                                        tau = RC$tau,
                                                        epsilon = RC$epsilon,
...
plm0.density_evaluation_known_c <- function(theta, RC) {
    if(is.null(RC$Q_me)){
        return(plm0_density_evaluation_known_c_cpp(theta = theta,
                                                   h = RC$h,
                                                   y = RC$y,
                                                   epsilon = RC$epsilon,
                                                   Sig_ab = RC$Sig_ab,
        ...
        return(plm0_me_density_evaluation_known_c_cpp(theta = theta,
                                                      h = RC$h,
                                                      y = RC$y,
                                                      tau = RC$tau,
                                                      epsilon = RC$epsilon,
...
plm0.predict_u_unknown_c <- function(theta, x, RC) {
    return(plm0_predict_u_unknown_c_cpp(theta = theta,
                                        x = x,
                                        h_u = RC$h_u,
                                        h_min = RC$h_min
    ))
...
plm0.predict_u_known_c <- function(theta, x, RC) {
    return(plm0_predict_u_known_c_cpp(theta = theta,
                                      x = x,
                                      h_u = RC$h_u,
                                      c = RC$c
    ))
...
plm0.calc_Dhat <- function(theta, RC) {
    theta_median <- sapply(1:nrow(theta), function(x) median(theta[x, ]))
    if (!is.null(RC$c)) {
