...
#' @return bgplm0 returns an object of class "bgplm0". An object of class "bgplm0" is a list containing the following components: \cr
...
#' }
#' @export
bgplm0 <- function(formula,data,c_param=NULL,h_max=NULL,parallel=T,forcepoint=rep(FALSE,nrow(data))){
    #argument checking
    ...
    MCMC_output_list <- bgplm0.inference(y=log(Q),h=h,c_param,h_max,parallel,forcepoint)
...
bgplm0.inference <- function(y,h,c_param=NULL,h_max=NULL,parallel=T,forcepoint=rep(FALSE,length(h)),num_chains=4,nr_iter=20000,burnin=2000,thin=5){
