...
gplm <- function(formula,data,c_param=NULL,h_max=NULL,parallel=T,forcepoint=rep(FALSE,nrow(data))){
  ...
  row.names(result_obj$param_summary) <- get_param_names('gplm',c_param)
  ...
  names(autocorrelation_df) <- get_param_names('gplm',c_param)
  autocorrelation_df$lag <- 1:nrow(autocorrelation_df)
  result_obj$autocorrelation <- autocorrelation_df[,c('lag',get_param_names('gplm',c_param))]
  result_obj$num_effective_samples <- data.frame(param=get_param_names('gplm',c_param),num_effective_samples=MCMC_output_list$num_effective_samples)
  result_obj$r_hat <- data.frame(param=get_param_names('gplm',c_param),r_hat=MCMC_output_list$r_hat)
...
gplm.inference <- function(y,h,c_param=NULL,h_max=NULL,parallel=T,forcepoint=rep(FALSE,length(h)),num_chains=4,nr_iter=20000,burnin=2000,thin=5){
  ...
  if(!is.null(RC$c)){
    density_fun <- gplm.density_evaluation_known_c
    unobserved_prediction_fun <- gplm.predict_u_known_c
  }else{
    density_fun <- gplm.density_evaluation_unknown_c
    unobserved_prediction_fun <- gplm.predict_u_unknown_c
  }
  ...
  RC$desired_output <- get_desired_output('gplm',RC)
