...
gplm <- function(formula, data, c_param = NULL, h_max = NULL, parallel = TRUE, num_cores = NULL, forcepoint = rep(FALSE, nrow(data)), verbose = TRUE){
    ...
    # Bayesian inference
    MCMC_output_list <- gplm.inference(y = log(Q), Q_me = Q_me, h = h, c_param = c_param, h_max = h_max, parallel = parallel, forcepoint = forcepoint, num_cores = num_cores, verbose = verbose)
...
gplm.inference <- function(y, Q_me, h, c_param = NULL, h_max = NULL, parallel = TRUE, forcepoint = rep(FALSE, length(h)), num_cores = NULL, num_chains = 4, nr_iter = 20000, burnin = 2000, thin = 5, verbose){
    ...
    RC <- get_model_components('gplm', y, Q_me, h, c_param, h_max, forcepoint, h_min = c_upper)
...
gplm.density_evaluation_unknown_c <- function(theta, RC) {
    if(is.null(RC$Q_me)){
        ...
        return( gplm_me_density_evaluation_unknown_c_cpp(theta = theta,
                                                         P = RC$P,
                                                         h = RC$h,
                                                         B = RC$B,
                                                         dist = RC$dist,
...
gplm.density_evaluation_known_c <- function(theta, RC) {
    if(is.null(RC$Q_me)){
        ...
        return(gplm_me_density_evaluation_known_c_cpp(theta = theta,
                                                      P = RC$P,
                                                      h = RC$h,
                                                      B = RC$B,
                                                      dist = RC$dist,
...
gplm.calc_Dhat <- function(theta, RC){
    ...
    if (is.null(RC$Q_me)) {
        ...
        mu <- (matMult(X, x))[1:RC$n, ]
        Dhat <- -2 * sum(dnorm(RC$y[1:RC$n, ], mu, sqrt(varr), log = TRUE))
    } else {
