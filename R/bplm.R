...
#' }
#' @export
bplm <- function(formula,data,c_param=NULL,h_max=NULL,parallel=T,forcepoint=rep(FALSE,nrow(data))){
    #argument checking
    ...
    MCMC_output_list <- bplm.inference(y=log(Q),h=h,c_param,h_max,parallel,forcepoint)
    result_obj=list()
...
bplm.inference <- function(y,h,c_param=NULL,h_max=NULL,parallel=T,forcepoint=rep(FALSE,length(h)),num_chains=4,nr_iter=20000,burnin=2000,thin=5){
