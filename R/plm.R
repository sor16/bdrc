#' Power Law Model
#'
#' plm is used to fit a discharge rating curve for paired measurements of stage and discharge using a Power Law Model with variance that may vary with stage as described in Hrafnkelsson et al. See "Details" for a more elaborate description of the model.
...
#' set.seed(1)
#' formula <- Q~W
#' plm.fit <- plm(formula,V316_river)
#' summary(plm.fit)
#' plot(plm.fit)
#' plm.fit_known_c <- plm(formula,V316_river,c_param=0.75,h_max=2)
...
plm <- function(formula,data,c_param=NULL,h_max=NULL,parallel=T,forcepoint=rep(FALSE,nrow(data))){
    ...
    row.names(result_obj$param_summary) <- get_param_names('plm',c_param)
    ...
    names(autocorrelation_df) <- get_param_names('plm',c_param)
    autocorrelation_df$lag <- 1:nrow(autocorrelation_df)
    result_obj$autocorrelation <- autocorrelation_df[,c('lag',get_param_names('plm',c_param))]
    result_obj$num_effective_samples <- data.frame(param=get_param_names('plm',c_param),num_effective_samples=MCMC_output_list$num_effective_samples)
    result_obj$r_hat <- data.frame(param=get_param_names('plm',c_param),r_hat=MCMC_output_list$r_hat)
...
plm.inference <- function(y,h,c_param=NULL,h_max=NULL,parallel=T,forcepoint=rep(FALSE,length(h)),num_chains=4,nr_iter=20000,burnin=2000,thin=5){
    ...
    if(!is.null(RC$c)){
        density_fun <- plm.density_evaluation_known_c
        unobserved_prediction_fun <- plm.predict_u_known_c
    }else{
        density_fun <- plm.density_evaluation_unknown_c
        unobserved_prediction_fun <- plm.predict_u_unknown_c
    }
