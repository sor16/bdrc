...
#'
#'Takes in name of a stage-discharge file, calculates a rating curve using \code{\link{model2BH}} and returns ggplot figures of custom outputs
...
plotmodel2 <- function(filename,eps=F,png=F,realscale=T,logscale=T,standardResiduals=T,realresiduals=T,beta=T){
    ...
    if(realscale==TRUE){
        rcreal=ggplot(ypodata)+theme_bw()+geom_point(data=realdata,aes(exp(Q),W))+geom_path(aes(exp(fit),W))+
